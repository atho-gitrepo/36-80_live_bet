...
class FirebaseManager:
    ...
    def get_unresolved_bets(self, bet_type=None):
        ...
        try:
            ...
            # Fixed log formatting (no confusing precedence)
            msg = f"Retrieved {len(result)} unresolved bets"
            if bet_type:
                msg += f" of type {bet_type}"
            logger.info(msg)
    ...
    def add_unresolved_bet(self, match_id, data):
        """Add unresolved bet with transaction to maintain consistency (idempotent)"""
        @firestore.transactional
        def add_bet_transaction(transaction, match_ref, unresolved_ref):
            # 1) Ensure tracked match exists
            ...
            # 2) If an unresolved bet already exists for this match_id and same bet_type, skip
            existing_unresolved = transaction.get(unresolved_ref)
            if existing_unresolved.exists:
                existing = existing_unresolved.to_dict() or {}
                if existing.get("bet_type") == data.get("bet_type"):
                    logger.info(
                        f"Unresolved bet already exists for match {match_id} "
                        f"with bet_type {data.get('bet_type')}; skipping (idempotent)."
                    )
                    return

            # 3) Create/update unresolved
            transaction.set(unresolved_ref, data)

            # 4) Update tracked match in the same transaction
            if "tracked_updates" in data and isinstance(data["tracked_updates"], dict):
    ...
    def cleanup_old_matches(self, days_threshold=7):
        ...
        try:
            ...
            query = self.db.collection('tracked_matches').where(
                filter=FieldFilter('last_update', '<', cutoff_date.isoformat())
            )
